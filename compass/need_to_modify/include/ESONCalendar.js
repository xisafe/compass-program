var _ESONCalendar = window.ESONCalendar = {
	...
	uanv_tool_CE: function(type, id, parent, className, HTML) {
		...
		if(obj.id == "ESONCalendar_Win"){
			return document.getElementById("elvis_right_content").appendChild(obj);
		}
	...
	onselect: function(d) {
		this.hotInput && (this.hotInput.value = d.y + this.splitChar + d.m + this.splitChar + d.d),
		this.hide()
	},
	addStyle: function() {
		...
		cssText += "#selMonth{margin-right:5px;width:100px}";
		...
		cssText += "#ESONCalendar_week th{font-size:12px;width:22px;height:18px;}";
	...
	showTo: function(obj) {
		...
		this.Win.style.top = (pos.y + 2 + oldObj.offsetHeight - 30) + "px";

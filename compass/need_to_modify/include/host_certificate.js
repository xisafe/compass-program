$(document).ready(function(){
    ...
    create_CA_add_panel = new RuleAddPanel( create_CA_add_panel_config );
    ...
    create_CA_add_panel.set_ass_list_panel( list_panel );
    list_panel.set_ass_add_panel( add_panel );
    
    add_panel.set_ass_message_manager( message_manager );
    create_CA_add_panel.set_ass_message_manager( message_manager );
...
var message_box_config = {
    url: "/cgi-bin/host_certificate.cgi",
        ...
        sub_items: [                /* ===可选===，想要添加当前没有考虑到的按钮，通过此选项 */
            {
                ...
                value: "取消",
                functions: {        /* ===可选===，定义对按钮进行相应操作时应当做的事，比如onclick、onmousehover */
                    onclick: "create_CA_add_panel.hide();"
                }
            },
            {
                enable:true,
...
var list_panel_config = {
    url: "/cgi-bin/host_certificate.cgi", /* ***必填***，控制数据在哪里加载 */
    ...
    panel_header: [{
        ...
        "enable": true,
        "type": "text",
        "title": "证书类型",        //一般text类型需要title,不然列表没有标题
...
function create_CA_add( element ) {
    create_CA_add_panel.show();
}
...
function add_CA_config(){
    $("#add_panel_body_form_id_for_create_CA_add_panel").submit();

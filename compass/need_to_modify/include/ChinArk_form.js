...
ChinArk_forms.prototype = {
	...
	_submit_check: function(obj, me, tb_id, div_id) {
		...
		var panel_popup_id = '#add_panel_popup_id_for_' + panel_name; //整个包裹层
		...
		if ($(panel_popup_id).length != 0) {
			error_tip = $(panel_popup_id+' [name="error_item"]');
	...
	_popup_tip:function(res,form_name,name,msg){
		...
		if (res == 'wrong') {
			...
			$(popup_item_id)/*.css({'left':ele_site.left+58,'top':ele_site.top-20})*/.show();

...
function generate_list_main_body_bak () {
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        for ( var j = 0; j < display_cols.length; j++) {
            table_main_body += '<td>';
            if(display_cols[j] == "virtual_server"){
                detail_data[i][display_cols[j]] = detail_data[i]["virtual_ip"]+":"+detail_data[i]["virtual_port"]+"("+detail_data[i]["virtual_protocol"]+")";
            }else if(display_cols[j] == "really_server"){
                detail_data[i][display_cols[j]] = detail_data[i]["really_ip"]+":"+detail_data[i]["really_port"]+"("+detail_data[i]["really_protocol"]+")";
            }else if(display_cols[j] == "logs_set"){
                if(detail_data[i]["access"] == "on"){
                    detail_data[i][display_cols[j]] = "访问";
                }
...
function generate_list_main_body() {
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        var datetime_num = detail_data[i].create_time;
        var datetime = datetime_num.substring(0,4)+"-"+datetime_num.substring(4,6)+"-"+datetime_num.substring(6,8)+" "
        +datetime_num.substring(8,10)+":"+datetime_num.substring(10,12)+":"+datetime_num.substring(12,14);
        //table_main_body += '<div><img style="float:left" class="action-image" src="../images/excel.png" onclick="file_view('+datetime_num+')"/>';
        //<a style="float:left" onclick=file_view('+datetime_num+')>预览</a>
        table_main_body += '<div><form style="margin:0px 0px" class="export-all-form" method="post" ACTION="/cgi-bin/logs_report_output.cgi"><a style="float:left" href="/cgi-bin/logs_report_output.cgi?ACTION=export_data&datetime='+datetime_num+'"><img class="action-image" src="../images/excel.png" /></a></form>';
        table_main_body += '<input type="image" class="action-image" src="../images/delete.png" title="删除" value="' + detail_data[i].line + '" onclick="delete_item(this);"/>';
        table_main_body += '</div></td>';
        odd_or_even++;
    }
...
function update_main_body_info() {
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        //actions += '<div><img style="float:left" class="action-image" src="../images/excel.png" onclick="file_view('+datetime_num+')"/>';
        //<a style="float:left" onclick=file_view('+datetime_num+')>预览</a>
        actions += '<div><form style="margin:0px 0px" class="export-all-form" method="post" ACTION="/cgi-bin/logs_report_output.cgi"><a style="float:left" href="/cgi-bin/logs_report_output.cgi?ACTION=export_data&datetime='+datetime_num+'"><img style="margin-top:5px" class="action-image" src="../images/excel.png" /></a></form>';
        actions += '<input type="image" style="margin-top:5px" class="action-image" src="../images/delete.png" title="删除" value="' + data_item_id + '" onclick="delete_item(this);"/></div>';
...
function edit_item(element) {
    ...
    var cbk_templates = document.getElementsByName("report_template");
    var templates = data_item.report_template.split("|");
    for(var i=0;i<templates.length;i++){
        for(var j=0;j<cbk_templates.length;j++){
            if(templates[i] == cbk_templates[j].value){
                cbk_templates[j].checked = true;
            }
        }
    }
    
    var cbk_cycles = document.getElementsByName("time_period");
    var cycle = data_item.time_period;
    for(var i=0;i<cbk_cycles.length;i++){
        if(cycle == cbk_cycles[i].value){
            cbk_cycles[i].checked = true;
        }
    }
    
    if(data_item.produce_format){
        document.getElementById("xsl").checked = true;
    }
    /*对表单中的值重新做检查*/
    ...
    function ondatareceived(data) {
        if(data == "not found"){
            ...
            try{
                ...
                wb = xlApp.Workbooks.open("https://"+url+data);
                xlSheet = wb.Worksheets("Sheet1");

...
function generate_list_main_body() {
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        for ( var j = 0; j < display_cols.length; j++) {
            table_main_body += '<td>';
            if(display_cols[j] == "create_time"){
                var create_time = detail_data[i][display_cols[j]].substring(0,4)+"-"+detail_data[i][display_cols[j]].substring(4,6)+"-"+detail_data[i][display_cols[j]].substring(6,8)+" "
                +detail_data[i][display_cols[j]].substring(8,10)+":"+detail_data[i][display_cols[j]].substring(10,12)+":"+detail_data[i][display_cols[j]].substring(12,14);
                table_main_body += create_time;
            }else{
                table_main_body += detail_data[i][display_cols[j]];
            }
        ...
        table_main_body += '<td class="action" colspan="2">';
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        for(var j = 0; j < display_cols.length; j++) {
            if(display_cols[j] == "create_time"){
                var create_time = detail_data[i][display_cols[j]].substring(0,4)+"-"+detail_data[i][display_cols[j]].substring(4,6)+"-"+detail_data[i][display_cols[j]].substring(6,8)+" "
                +detail_data[i][display_cols[j]].substring(8,10)+":"+detail_data[i][display_cols[j]].substring(10,12)+":"+detail_data[i][display_cols[j]].substring(12,14);
                listb_tr.eq(tr_num).find("td").eq( j ).html(create_time);
            }else{

...
function distribute_strategy() {
    ...
    for(var i = 0; i < items_length; i++) {
        if(detail_data[i].checked) {
            engines.push( detail_data[i].line );
        }
    }
...
function power_down_item(element){
    var data_item = get_data_item( element.value );
    if ( data_item.engine_connected != "CONNECTED" ) {
        /*未连接不能操作,直接返回*/
        show_warn_mesg("设备未连接，操作失败");
        return;
    }
    if(confirm("确认关闭?")) {
...
function count_selected_engine() {
    var detail_data = paging_holder.data_content.detail_data;
    var items_length = detail_data.length;
    var selected_num = 0;
    for(var i = 0; i < items_length; i++) {

...
// });
function new_init() {
    if( new_paging_holder.data_content.total <= 0 ) {
        /*数据从来没有加载或者没有数据，才重新请求*/
        new_load_data();
...
function new_load_data(){
    ...
    function ondatareceived(data) {
        ...
        new_paging_holder.data_content.detail_data = data_content.detail_data;
        new_paging_holder.data_content.total = data_content.total;
        new_paging_holder.data_content.display_cols = data_content.display_cols;
        update_new_paging_holder();
        if(is_need_reload(data_content.reload)) {
            show_apply_mesg_box();
        }
        window.setTimeout("hide_loading_mesg()",10);//一般加载数据很快，这里做用户体验，让用户能看见加载图片
    }

    new_do_request(sending_data, ondatareceived);
}

function update_new_paging_holder() {
    var total = new_paging_holder.data_content.total;
    var page_size = new_paging_holder.page_size;
    /*Ceil computing*/
    ...
    var current_page = new_paging_holder.current_page;
    if (current_page <= 0){
        current_page = 1;
    ...
    /*Update new_paging_holder*/
    new_paging_holder.total_page = total_page;
    new_paging_holder.current_page = current_page;
    new_paging_holder.from_num = from_num;
    new_paging_holder.to_num = to_num;
}
...
function new_update_main_body_info() {
    /*这个函数作为备用方案*/
    var current_page = new_paging_holder.current_page;
    var total_page = new_paging_holder.total_page;
    var total = new_paging_holder.data_content.total;
    var from_num = new_paging_holder.from_num;/*Some difference*/
    var to_num = new_paging_holder.to_num;
    var page_size = new_paging_holder.page_size;
    var display_cols = new_paging_holder.data_content.display_cols;
    var detail_data = new_paging_holder.data_content.detail_data;
    var selected_line = new_paging_holder.selected_line;
    var table_main_body = "";
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        listb_tr.eq(tr_num).find("td").eq( 0 ).html('<input type="radio" name="strategy_item" class="new-rule-listbcc" value="' + data_item_id + '" onclick="new_toggle_item_check(this);" ' + checked + '/>');
...
function new_update_paging_tools_info(){
    ...
    var total_page = new_paging_holder.total_page;
    var total = new_paging_holder.data_content.total;
    var from_num = new_paging_holder.from_num + 1;/*Some difference*/
    var to_num = new_paging_holder.to_num;
...
function new_update_info() {
    ...
    // new_reset_check_all_box();现在没有全选按钮了
}
...
function new_set_check_all(status) {
    ...
    var from_num = new_paging_holder.from_num;
    var to_num = new_paging_holder.to_num;
    for (var i = from_num; i < to_num; i++){
        checkboxs[i - from_num].checked = status;
        new_paging_holder.data_content.detail_data[i].checked = checkboxs[i - from_num].checked;
    }
}

function new_toggle_item_check(element){
    /*Record the only one checked item by wl 2014.05.28*/
    for(var i = 0; i < new_paging_holder.data_content.detail_data.length; i++){
        if(new_paging_holder.data_content.detail_data[i].line == element.value){
            new_paging_holder.data_content.detail_data[i].checked = true;
        } else {
            new_paging_holder.data_content.detail_data[i].checked = false;
        }
    }
}

function new_select_edit_item_line(element){
    new_paging_holder.selected_line = element.value;
    new_update_main_body_info();
}
...
function new_first_page() {
    ...
    new_paging_holder.current_page = new_paging_holder.first_page;
    new_update_info();
    new_disable_first_last_page_op();
    /*To handle the condition that there is only one page*/
    if(new_paging_holder.current_page == new_paging_holder.total_page) {
        new_disable_next_end_page_op();
    }
}

function new_last_page() {
    if(new_paging_holder.current_page > new_paging_holder.first_page) {
        if(new_paging_holder.current_page == new_paging_holder.total_page) {
            new_enable_next_end_page_op();
        }
        new_paging_holder.current_page = new_paging_holder.current_page - 1;
        new_update_info();
        if(new_paging_holder.current_page == new_paging_holder.first_page){
            new_disable_first_last_page_op();
        }
...
function new_end_page() {
    new_enable_first_last_page_op();
    /*Set current page number to number one*/
    new_paging_holder.current_page = new_paging_holder.total_page;
    new_update_info();
    new_disable_next_end_page_op();
    /*To handle the condition that there is only one page*/
    if(new_paging_holder.current_page == new_paging_holder.first_page) {
...
function new_refresh_page() {
    if(new_paging_holder.selected_line >= 0) {
    ...
    if(new_paging_holder.current_page == new_paging_holder.first_page) {
        new_first_page();
    } else if(new_paging_holder.current_page == new_paging_holder.total_page) {
        new_end_page();
    } else {
        new_enable_first_last_page_op();
        new_enable_next_end_page_op();
...
function new_go_to_page(page) {
    if(page < new_paging_holder.first_page){
        $("#new-current-page").val(new_paging_holder.first_page);
        new_first_page();
    } else if(page > new_paging_holder.total_page){
        $("#new-current-page").val(new_paging_holder.first_page);
        new_end_page();
    } else {
        new_enable_first_last_page_op();
        new_enable_next_end_page_op();
        new_paging_holder.current_page = page;
        new_update_info();
    }
}
...
function new_reset_selected_line() {
        /*重置选中的行*/
        new_paging_holder.selected_line = -1;
}

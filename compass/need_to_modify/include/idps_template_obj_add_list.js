...
function generate_list_main_body() {
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        table_main_body += '</td>';
        for ( var j = 0; j < display_cols.length; j++) {
            if(display_cols[j] == 'addr_value'){
                table_main_body += '<td>';
                if(detail_data[i]['addr_type'] == 'addr'){
                    table_main_body += detail_data[i][display_cols[j]];
                }else if(detail_data[i]['addr_type'] == 'subnet'){
                    table_main_body += (detail_data[i][display_cols[j]]+'/'+detail_data[i]['addr_value_another']);
                }else if(detail_data[i]['addr_type'] == 'iprange'){
                    table_main_body += (detail_data[i][display_cols[j]]+'-'+detail_data[i]['addr_value_another']);
                }
                table_main_body += '</td>';
            }else if(display_cols[j] == 'addr_type'){
                table_main_body += '<td>';
                if(detail_data[i]['addr_type'] == 'addr'){
                    table_main_body += "IP地址";
                }else{
                    table_main_body += "IP子网";
                }
                table_main_body += '</td>';
            }else{
                table_main_body += '<td>';
        ...
        for(var j = 0; j < display_cols.length; j++) {
            if(display_cols[j] == 'addr_value'){
                if(detail_data[i]['addr_type'] == 'addr'){
                    ...
                    listb_tr.eq(tr_num).find("td").eq( j+1 ).html(detail_data[i][display_cols[j]]+'/'+detail_data[i]['addr_value_another']);
                }else if(detail_data[i]['addr_type'] == 'iprange'){
                    listb_tr.eq(tr_num).find("td").eq( j+1 ).html(detail_data[i][display_cols[j]]+'-'+detail_data[i]['addr_value_another']);
                }
            }else if(display_cols[j] == 'addr_type'){
                if(detail_data[i]['addr_type'] == 'addr'){
                    listb_tr.eq(tr_num).find("td").eq( j+1 ).html("IP地址");
                }else if(detail_data[i]['addr_type'] == 'subnet'){
                    listb_tr.eq(tr_num).find("td").eq( j+1 ).html("IP子网");
                }
            }else{
                listb_tr.eq(tr_num).find("td").eq( j+1 ).html(detail_data[i][display_cols[j]]);
            }
...
function edit_item(element) {
    ...
    if(data_item.addr_type == "addr"){
        document.getElementById("name_addr_value").innerHTML="IP地址";
        document.getElementById("td_addr_value").innerHTML = '<input style="margin-left:4px;float:left" name="addr_value" id = "addr_val" oninput="isip(this)" onpropertychange="isip(this)" class="input-text" type="text"><div style="float:left;color:red" id="msg"></div>';
        document.getElementById("addr_val").value = data_item.addr_value;
    }else if(data_item.addr_type == "subnet"){
        document.getElementById("name_addr_value").innerHTML="IP子网";
        document.getElementById("td_addr_value").innerHTML = '<input style="margin-left:4px;float:left" name ="subnet_value" id = "subnet_val" class="input-text" type="text" oninput="isip_net(this)" onpropertychange="isip_net(this)"><div style="float:left;color:red" id="msg_net"></div>';
        document.getElementById("subnet_val").value = data_item.addr_value+"/"+data_item.addr_value_another;
    }else if(data_item.addr_type == "iprange"){
        document.getElementById("name_addr_value").innerHTML="IP范围";
        document.getElementById("td_addr_value").innerHTML = '<div style="float:left"><input style="width:93px" oninput="isip(this)" onpropertychange="isip(this)" name = "ip_from" id = "ip_f" type="text"> - <input style="width:93px" oninput="isip(this)" onpropertychange="isip(this)" name="ip_to" id = "ip_t" type="text"></div> <div style="float:left;color:red" id="msg_iprangge"></div>';
        var ip_array = data_item.addr_value.split("-");
        document.getElementById("ip_f").value = ip_array[0];
        document.getElementById("ip_t").value = ip_array[1];
    }
    
    document.getElementById("des").value = data_item.description;
...
function save_data(url) {
    ...
    if(check._submit_check(object,check)){
        ...
        /* var addr_type = document.getElementById("addr_tp").value;
        if(addr_type == 'addr'){
            sending_data+="&addr_value="+document.getElementById("addr_val").value;
        }else if(addr_type == 'subnet'){
            sending_data+="&subnet_value="+document.getElementById("subnet_val").value;
        }else if(addr_type == 'iprange'){
            sending_data+="&ip_f="+document.getElementById("ip_f").value;
            sending_data+="&ip_t="+document.getElementById("ip_t").value;
        }*/

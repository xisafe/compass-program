...
var list_history_events_config = {
    ...
    event_handler: {
        ...
        after_load_data: function ( list_obj, response ) {
            //  var len = response.detail_data.length;
            // for(var i=0;i<len;i++){
            //     if(response.detail_data[i].src_user==""){

            //     }
            //     else{
            //          response.detail_data[i].sip =  response.detail_data[i].sip +response.detail_data[i].sport+ "("+ response.detail_data[i].src_user+")";
            //     }
            //     if(response.detail_data[i].dst_user==""){

            //     }
            //     else{
            //         response.detail_data[i].dip =  response.detail_data[i].dip +response.detail_data[i].d+ "("+ response.detail_data[i].dst_user+")";    
            //     }
            // }
        }
    },
...
function load_data_into_template_add( add_window, data_item ) {
    ...
    if ( data_item.time_range.type == 2 ) {
        ...
        var regx = /(last(?:day|week|mouth)_)(\d+)/;
        if ( time.match( regx ) ) {

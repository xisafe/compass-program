...
sub init_data(){
    $custom_dir         = '/virtual_twine';
    $conf_dir           = "${swroot}".$custom_dir;
    $conf_file          = $conf_dir.'/config';
    $need_reload_tag    = $conf_dir.'/add_list_need_reload';

    $getInterface       ='/usr/local/bin/getinterface.py';
    $cmd                ='/usr/local/bin/setvirtualtwine.py';
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/virtual_twine.js"></script>';
...
sub get_config_hash($) {
    ...
    $config{'name'}         = $temp[0];
    $config{'twineOne'}     = $temp[1];
    $config{'twineTwo'}     = $temp[2];
...
sub get_config_record($) {
    ...
    push( @record_items, $hash_ref->{'twineOne'} );
    push( @record_items, $hash_ref->{'twineTwo'} );

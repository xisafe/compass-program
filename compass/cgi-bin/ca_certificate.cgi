...
sub init_data(){
    ...
    $conf_file          = $conf_dir.'/ca/ca_config';              #发件人信息所存放的文件
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/ca_certificate.js"></script>';
...
sub upload_file(){
    ...
    # system("echo $filename>/var/efw/chensisi.debug");
    ...
    my $temp_cmd = "sudo /usr/local/bin/import_ca_certification.py  -n $filename -f $file_path";
    # system("$temp_cmd");
    # print $filename;
    # print $temp_cmd;

    my $status = `$temp_cmd`;
    # system("echo $status>/var/efw/chensisi.debug");
    if ($status eq '0') {
        $notemessage = "导入成功";
    }
    else {
...
sub delete_data() {
    ...
    my $del_cmd;
    my $r = join('|', @del_id);
    $del_cmd = "/usr/local/bin/delete_certification.py -t ca -i ".'"'.$r.'"';
    my $mesg;
    my $status = `$del_cmd`;
    if ($status eq '0') {
        $mesg = '删除成功';
...
sub get_config_hash($) {
    ...
    $config{'nameForCA'}    = $temp[1];
    $config{'authorizer'}   = $temp[2];
    $config{'theme'}        = $temp[3];

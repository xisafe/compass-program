...
our @RRDDefaultArgs = ('-w', '695','-h', '140','--color', 'SHADEA#a7e1f7','--color', 'SHADEB#4fb0c1','--color', 'BACK#f5fbfc',);
...
for (qw(action host plugin plugin_instance type type_instance timespan ini_title))
{
	$Args->{$_} = param ($_);
}
...
sub action_show_graph
{
  ...
  for (my $i = 0; $i < @DataDirs; $i++)
  {
    ...
    RRDs::graph ('-', '-a', 'PNG', '-s', $start_time, '-t',_($ini_title), @RRDDefaultArgs, @rrd_args);
		#RRDs::graph ("-","-a", "PNG", "-s", "-i", "-z",
		#"--alt-y-grid", "-w 900", "-h 180", "-l 0", "-u 100", "-r",
		#"--color", "SHADEA#a7e1f7",
		#"--color", "SHADEB#4fb0c1",
		#"--color", "BACK#f5fbfc");
	print FILE "$file\n";
...
sub main
{
	  ...
	  && ($Args->{'action'} eq 'show_graph'))
	{
	  ...
	  action_show_graph ($Args->{'host'},
	    ...
	    $Args->{'type'}, $Args->{'type_instance'},$Args->{'ini_title'});
	  return (0);
	}

...
my $setting_file_dynamic;
my $setting_file_static;
my $page_title;         #页面标题
my $extraheader;        #存放用户自定义JS
my %par;                #存放post方法传过来的数据的哈希
my %query;              #存放通过get方法传过来的数据的哈希
my %settings_dynamic;   #存放该页面配置的哈希
my %settings_static;    #存放该页面配置的哈希
...
sub init_data(){
    $custom_dir         = '/route';                      #要保存数据的目录名字,要配置,不然路径初始化会出问题
    $conf_dir           = "/var/log".$custom_dir;          #规则所存放的文件夹
    $conf_file          = $conf_dir.'/multicastrouteinfo';              #发件人信息所存放的文件
    $setting_file_dynamic= "/var/efw/multicastrouting/dynamicmulticast/settings";
    $setting_file_static= "/var/efw/multicastrouting/staticmulticast/settings";
    $cmd                = "/usr/local/bin/Getmulticastrouting";
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/multicastrouteinfo_init.js"></script>';
...
sub display_list_div() {
    printf<<EOF
    <div id="panel_multicastroute_list" style="width: 96%;margin: 20px auto;"></div>
EOF
    ;
}
...
sub get_config_hash($) {
    ...
    $config{'input_interface'}   = $temp[2];
    $config{'output_interface'}   = $temp[3];
    #============自定义字段组装-END===========================#
    return %config;
}
...
sub update_data(){
    system($cmd);
    #readhash($setting_file_dynamic, \%settings_dynamic);
    &readhash($setting_file_static, \%settings_static);
    my %hash;
    #%hash->{'state_dynamic'} = $settings_dynamic{'ENABLED_DYNAMIC'};
    %hash->{'state_static'} = $settings_static{'ENABLED_STATIC'};
    my $result = $json->encode(\%hash);
    print $result;
}
...
sub init_data(){
    ...
    $custom_dir         = '/objects/file_type';                            #要保存数据的目录名字,要配置,不然路径初始化会出问题
    $conf_dir           = "/var/efw".$custom_dir;                    #规则所存放的文件夹
    $conf_file          = $conf_dir.'/fileconfig';              #策略模板所存放的文件
    $pid_file           = $conf_dir.'/pid';
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/file_type_group.js"></script>';
...
sub display_list_div() {
	...
	<div id="detail_panel" style="width: 96%;margin: 20px auto;"></div>
...
sub get_config_record($) {
    ...
    my $flags = 1;
    push( @record_items, $flags);

    my $extentions = $hash_ref->{'extentions'};
    $extentions =~ s/\r//g;
    $extentions =~ s/\n/\|/g;
    push( @record_items, $extentions);

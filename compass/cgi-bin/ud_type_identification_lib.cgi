...
my $extraheader     = '<link rel="stylesheet" type="text/css" href="/include/ud_type_identification_lib.css"/>
                        ...
                        <script type="text/javascript" src="/include/ud_type_identification_lib.js"></script>';
my %par;
my %settings;
my %system_settings;
...
sub doaction() {
    ...
    if($par{'update_now'} ne ''){
        my $temp_cmd = "sudo /usr/local/bin/online_rules_update.py -t app_rule -n online_app_rule.dat";
        my $res = `$temp_cmd `;
        if($res eq "yes"){
                $notemessage ="升级成功!";
            }else{

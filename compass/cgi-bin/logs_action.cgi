...
sub init_data() {
    ...
    $logname          ="network.log_new";
    $logfile          ="/var/log/network.log_new";
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/logs_action.js"></script>';
...
sub  do_action() {
    ...
    if ( $action eq 'load_data' && $panel_name eq 'list_panel' ) {
        ...
        ($errormessage,$notemessage) = &importLog(\%par,'network.log_new');
        &show_page();
    }       
...
sub get_config_hash($) {
    ...
    $timestamp = change_time($temp[0]);
    $config{'datetime'}        = $timestamp;
    $config{'messages'}        = $temp[1];
...
sub clearLogForPass(){
    my %ret_data;
    my $filestr = 'clear='.'network.log_new';
...
sub save_data() {
    ...
    my @search_arr = &get_config_record( \%par );
    my ( $status, $error_mesg, $total_num ) = &read_config_lines( $logfile, \@current_data ); 

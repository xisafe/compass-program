...
#记录日志应用
my $CUR_PAGE = "自定义应用" ;  #当前页面名称，用于记录日志
...
sub init_data(){
    $custom_dir         = '/objects/application';
    $conf_dir           = "${swroot}".$custom_dir;
    $conf_file          = $conf_dir.'/app_custom';
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    <link rel="stylesheet" type="text/css" href="/include/ob_se_custom_service.css" />
                    <script language="JavaScript" src="/include/message_manager.js"></script>
                    <script language="JavaScript" src="/include/add_panel.js"></script>
                    <script language="JavaScript" src="/include/list_panel.js"></script>
                    <script language="JavaScript" src="/include/ob_se_custom_service.js"></script>';
...
sub get_config_hash($) {
	...
	$config{'name'}       = $temp[1];
	$config{'string'}     = $temp[3];
    # my $description = $temp[3];
    # $description =~ s/\&/\n/g;
...
sub get_config_record($) {
    ...
    my $string = $hash_ref->{'string'};
    $string =~ s/\n/\&/g;
    # $string =~ s/\r/\&/g;

    push( @record_items, $string );
...
sub save_data_handler() {
    ...
    if ( $par{'id'} ne "" ) {
        ...
        $par{'serviceId'} = $exist_record_hash{'serviceId'};

...
sub init_data(){
    $custom_dir = 'ips/port_custom';                            #要保存数据的目录名字 ！！！！要配置，不然路径初始化出问题！！！！
    ...
    $export_file = '/tmp/access_logs_list.csv';
    $export_filename = 'access_logs_list.csv';
    $waf = '/usr/local/nginx/conf/waf.py';
    $restart = '/usr/local/bin/restartips';                                       #应用重启的程序,根据实际情况填写
    $extraheader = '<script language="JavaScript" src="/include/idps_port_custom_init.js"></script>
                    <script language="JavaScript" src="/include/idps_port_custom_list.js"></script>
...
sub save_data_edt(){
    my $name_cn = $par{'name_save'};
    my @lines_old = read_conf_file($conf_file);
    my @lines_new;
    foreach(@lines_old){
        my @line_data = split(",",$_);
        if($name_cn eq $line_data[1]){
            $line_data[2] = $par{'port_save'};
            my $line_new = join(",",@line_data);
            push(@lines_new,$line_new);
        }else{
            push(@lines_new,$_);
        }
    }
    &save_conf_file(\@lines_new,$conf_file);
    my $status = 0;
    my $reload = 1;
    my $mesg = "修改信息成功";
    &reset_values();
...
sub generate_download_file() {
    ...
    my @new_lines;
    # my $temp = "07/May/2014:11:31:20,192.168.4.117,GET,192.168.4.172:80,/answer.html,403,607,text/htm,http://192.168.4.172/";
    # push(@new_lines, $temp);
...
sub load_data(){
    ...
    for(my $i = 0; $i < $record_num; $i++){
        ...
        if($search ne ""){
            ...
            my $fiter_field = $data[$condition];
            my $new_field = lc($fiter_field);
    ...
    <div id="add-div" >
        <div id="add-div-header">
            <span class="add-header-title">
                ...
                <span id="add-title" class="label-text">添加服务器策略</span>
            </span>
        </div>
...
sub config_add_item_data() {
    my %hash = (
        ...
        "width" => "30%",#此字段只有在is_in_list字段不等于0时可用，并且所有的启用了的此字段加起来为80%最适宜，可微调
        "class" => "",
    );
...
sub create_display_add_lines(){
    ...
    for(my $i = 0; $i < $total_count; $i++){
        ...
        if($item->{'type'} eq 'setting'){
            if($item->{'name'} eq 'virtual_server'){
                printf <<EOF
                <tr class="$bgcolor">
                    <td class="add-div-type" rowspan="3">$item->{'display'}</td>
                    <td>IP*</td>
                    <td><input type="text" name="virtual_ip" class="input-text"/></td>
                </tr>
                <tr class="$bgcolor">
                   <td>端口号*</td>
                   <td><input type="text" name="virtual_port" class="input-text"/></td>
                </tr>
                <tr class="$bgcolor">
                   <td>协议类型</td>
                   <td><select class="input-select" name="virtual_protocol">
...
EOF
        ;
            }elsif($item->{'name'} eq 'really_server'){
                printf <<EOF
                <tr class="$bgcolor">
                    <td class="add-div-type" rowspan="3">$item->{'display'}</td>
                    <td>IP*</td>
                    <td><input type="text" name="really_ip" class="input-text"/></td>
                </tr>
                <tr class="$bgcolor">
                   <td>端口号*</td>
                   <td><input type="text" name="really_port" class="input-text"/></td>
                </tr>
                <tr class="$bgcolor">
                   <td>协议类型</td>
                   <td><select class="input-select" name="really_protocol">
                            <option value="HTTP">HTTP</option>
                        </select>
                   </td>
                </tr>
EOF
        ;
            }elsif($item->{'name'} eq 'logs_set'){
                printf <<EOF
                <tr class="$bgcolor">
                    <td class="add-div-type" colspan="2">$item->{'display'}</td>
                    <td><input name="access" type="checkbox" value="on"/>&nbsp;&nbsp;启用访问日志</td>
                </tr>               
EOF
        ;
            }
        }elsif($item->{'type'} eq 'hide'){
...
sub display_rule_list() {
    ...
    <div id= "bgDiv2" style= "width:100%;margin-left:300px; height:auto; position:absolute;display:none; top:150px; left:0px; filter:progid:DXImagesTransform.Microsoft.Alpha(style=3,opacity=25,finishOpacity=75); opacity:1.0; left:0; width:50%; z-index:10000;"> 
        ...
        <h1>编辑</h1>
        <span>名称：<input id="name_edt" class="input-text" type="text" readonly background-color="#dddddd"/></span>
        <span>端口：<textarea id="port_edt" class="input-textarea" type="textarea"></textarea></span>
        <div><input class='net_button' style="margin-top:9px;margin-left:44px" type='button' style='color: black; background-image: url(\"/images/button.jpg\");' value='确定' onclick="save_data_edt()"  name='save_edt' />
        <input class='net_button' style="margin-top:9px" type='button' style='color: black; background-image: url(\"/images/button.jpg\");' value='取消' onclick="cancel()" name='cancel' />
        </div>
        </div>
        <div class="guanbi">
        <label>
    ...
    <!--<div class="fd" id="detailDiv">
    <div class="fd_box">
    <div class="tm_box"></div>
    <div class="denglu_box">
    <h1>编辑</h1>
    <span>名称：<input id="name_edt" class="input-text" type="text" readonly /></span>
    <span>端口：<textarea id="port_edt" class="input-textarea" type="textarea"></textarea></span>
    <div><input class='net_button' style="margin-top:9px;margin-left:44px" type='button' style='color: black; background-image: url(\"/images/button.jpg\");' value='确定' onclick="save_data_edt()"  name='save_edt' />
    <input class='net_button' style="margin-top:9px" type='button' style='color: black; background-image: url(\"/images/button.jpg\");' value='取消' onclick="cancel()" name='cancel' />
    </div>
                    ...
                    <tr>
                        <td id="rule-list-foot-td" colspan="$colspan">
                            <!--<span class="opt-tools">
                                <input class='net_button' style="margin-top:9px" type='submit' style='color: black; background-image: url(\"/images/button.jpg\");' value='删除日志' onclick="warning_box('$filestr_change')" name='routebutton'>

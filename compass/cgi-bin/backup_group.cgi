...
my $conf_file_interface;#规则所存放的文件
...
sub init_data(){
    ...
    $conf_file_interface = $conf_dir.'/devpriority';                    #规则信息所存放的文件
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/backup_group_init.js"></script>';
...
sub display_list_div() {
    ...
    <div id="panel_backup_add" style="width: 96%;margin: 20px auto;"></div>
    <div id="panel_backup_list" style="width: 96%;margin: 20px auto;"></div>
EOF
    ;
}
...
sub load_data(){
    ...
    my @eths = &get_interfaces_eth();

    %ret_data->{'detail_data'} = \@content_array;
    %ret_data->{'interfaces'} = \@interfaces;
    %ret_data->{'eths'} = \@eths;
...
sub get_config_record($) {
    ...
    my $virtual_ip = $hash_ref->{'virtual_ip'};
    $virtual_ip =~ s/\r//g;
    $virtual_ip =~ s/\n/&/g;
    push( @record_items, $virtual_ip );
    push( @record_items, $hash_ref->{'interface_backupgroup'} );
    my $interface_listenner = $hash_ref->{'interface_listenner_save'};
    $interface_listenner =~ s/\|/&/g;
    push( @record_items, $interface_listenner );
...
sub get_interfaces(){
    ...
    my @lines = ();
    my @interfaces = ();
    &read_config_lines($conf_file_interface,\@lines);
    foreach(@lines){
        my @data_line = split(",",$_);
        push(@interfaces,$data_line[0]);
    }
    
    return @interfaces;
}

sub get_interfaces_eth(){
    my @all_hash;
    my $temp_cmd = `ip a`;
    my @temp = split(/\n/,$temp_cmd);

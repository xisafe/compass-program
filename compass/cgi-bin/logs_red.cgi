...
#
# red.log_new=> ABC
# 删除 ABC: rm /var/log/ABC -f
# 删除 ABC的历史日志： rm /var/log/ABC-*.gz -f
...
my $logfile="/var/log/red.log_new";
...
#日志导入功能
&importLog(\%cgiparams, 'red.log_new');

#日志清空功能
&clearLog(\%cgiparams, 'red.log_new');
...
if (!$skip) {
	...
	if (($filter !~ /^$/) && ($packet !~ /$filter/) && ($tmp[0] !~ /$filter/)) {
	    next;
	}
	$log[$lines] = $line;
	$lines++;
...
<tr>
    ...
    <td width='10%' align='center' class='boldbase'><b>结果</b></td>
</tr>
END
,
_('Time'),
...
<table width='100%' align='center'>
	<tr>
	<form enctype='multipart/form-data' method='post' action='$ENV{'SCRIPT_NAME'}'>
	<td width='140px'>%s: <input type="text" size="18" name='FILTER' VALUE="$filter" style="width:80px;"></td>
    <td width='220px'>%s: <input type="text" SIZE="12" id="inputDate" name='DATE' VALUE="$date" />

...
my $logfile="/var/log/openvpn/openvpn.log_new";
...
if ($cgiparams{'status'} eq 'down') {
	if ($filter) {
		if ($filestr =~ /\.gz$/) {
			...
			$tempd = `gzip -dc $filestr |grep "$filter_search" |sort -r|head -n $heads |tail -n $tails`; 
		}
		else{		
			...
			$tempd=`tac $filestr |grep "$filter_search" -m  $heads|head -n $heads |tail -n $tails`;	
		}		
	}
	else{
		if ($filestr =~ /\.gz$/) {
	...
	if ($filter) {
		if ($filestr =~ /\.gz$/) {
			...
			$tempd = `gzip -dc $filestr |grep "$filter_search" |head -n $heads |tail -n $tails`; 
		}
		else{
			$get_line_num=`grep "$filter_search" $filestr |wc -l`;
			if ($heads > $get_line_num) {
			...
			$tempd=`cat $filestr |grep "$filter_search" -m  $heads|head -n $heads |tail -n $tails`;		
		}
	}
	else{
		if ($filestr =~ /\.gz$/) {
...
<tr>
    <td width='15%' class='boldbase'><b>%s</b> $sort_time</td>
...
foreach my $line (@slice) {
	...
	my $packet = _($tmp[1],$tmp[3],$tmp[4],$tmp[5],$tmp[6],$tmp[7]);

...
my $extraheader     = '<link rel="stylesheet" type="text/css" href="/include/ud_hole_feature_lib.css"/>
                        ...
                        <script type="text/javascript" src="/include/ud_hole_feature_lib.js"></script>';
my %par;
my %settings;
my %system_settings;
...
sub getVersion(){ #获取版本信息#
    my $temp_cmd = "sudo /usr/local/bin/getUpdateRelease.py -t hole_rule";
...
sub doaction() {
    ...
    if($par{'down-load'} ne ''){ #在线升级点击下载#
        ...
        if($self_url =~ /[a-zA-Z0-9:.\/]/){
            system("echo $self_url > /var/efw/unify_update/hole_rule/address");

            my $temp_cmd = "sudo /usr/local/bin/downloadUpdatePakage.py -u $self_url -n online_hole_rule.dat";
            my $res = `$temp_cmd`;
            if($res eq "yes"){
                $notemessage ="下载成功!";
            }else{
    ...
    if($par{'update_now'} ne ''){
        my $temp_cmd = "sudo /usr/local/bin/online_rules_update.py -t hole_rule -n online_hole_rule.dat";
        my $res = `$temp_cmd `;
        if($res eq "yes"){
                $notemessage ="升级成功!";
            }else{
    ...
    if($par{'import_rules'} ne '') {
        ...
        if(!$file || $file !~ /\.dat/) {
            ...
            if(!open(UPLOAD, ">$tmp_dir/HoleFeatureLib.dat")) { $errormessage="打开文件失败"; return false; } 
            binmode UPLOAD;
            my $count = 0;
           ...
           my $temp_cmd = "sudo /usr/local/bin/unify_rules_update.py -t hole_rule -n HoleFeatureLib.dat ";
           my $res = `$temp_cmd`;
            if($res eq "yes"){
                $notemessage ="升级成功!";
            }else{
...
sub show_update_online(){
    ...
    my $self_url = `cat /var/efw/unify_update/hole_rule/address`;
    printf <<EOF

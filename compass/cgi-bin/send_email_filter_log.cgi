...
sub init_data(){
    $access_log_file_path = "/var/log/maillog";
    $base_dir = "/var/log/";
    $common_cmd = "/usr/local/bin/queryfilterlog.py";
    $log_search_tmp_file_path = "/tmp/smtp.log";
    #============扩展的CSS和JS文件-BEGIN========================================================#
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    <script language="JavaScript" src="/include/message_manager.js"></script>
                    <script language="JavaScript" src="/include/list_panel.js"></script>
                    <script language="JavaScript" src="/include/send_email_filter_log.js"></script>
...
sub show_page() {
    &openpage( "发送邮件过滤日志", 1, $extraheader );
...
sub load_data(){
    ...
    if( $par{'INIT_TAG'} eq 'true' ){
        ...
        `echo 222 >/tmp/haha`;
        if( $par{'search'} ne ''){
            $query_cmd = $common_cmd." -s -m ".$formated_date." -g ".$par{'search'};
            `echo "$query_cmd" >>/tmp/haha`;  # /usr/local/bin/queryfilterlog.py -t -m 20150510 -g baidu
            system($query_cmd);
        }
        else{
            $query_cmd = $common_cmd." -s -m ".$formated_date;
...
sub get_config_hash($){
    ...
    $config{'SENDER_EMAIL'}   = $temp[3];
    $config{'RECIPIENT_EMAIL'}  = $temp[4];
    $config{'DETAILS'}  = $temp[5];
...
sub delete_data(){
    ...
    my $formated_date = &get_format_date($par{'DATE'});
    my $cmd = $common_cmd." -s -e -m ".$formated_date;
    system($cmd);
    if($? == 0){
        $status = 0;
        $mesg = "日志删除成功";
...
sub export_data(){
    ...
    if( &is_today($formated_date) ){   `echo is_today-is-true >>/tmp/haha`;
        $export_file_path = $base_dir."maillog";
        $file_name = "maillog";
    }
    else{
        $export_file_path = $base_dir."maillog-".$formated_date.".gz";
        $file_name = "maillog-".$formated_date.".gz";
    }

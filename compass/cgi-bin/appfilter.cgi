...
sub load_data(){
    ...
    %ret_data->{'is_active'} = `sudo $cmd_set_appctrlIsactive`;
...
sub get_detail_data($) {
    ...
    for( my $i = 0; $i < @lines; $i++ ) {
		...
		# if( $hash{'action_permission'} == 3 ) {
			# $hash{'actionView'} = '拒绝';
		# }
	     if($search ne ""){
                if ($hash{'name'} !~ m/$search/ &&
                 $hash{'IpOrUser'} !~ m/$search/ &&
                  $hash{'Dest_IP_Group'} !~ m/$search/ &&
                  $hash{'ServiceOrApp'} !~ m/$search/

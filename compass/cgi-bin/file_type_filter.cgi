...
my $CUR_PAGE = "文件类型过滤" ;  #当前页面名称，用于记录日志
...
sub init_data(){
    $custom_dir         = '/filetypefilter';                             #要保存数据的目录名字,要配置,不然路径初始化会出问题
    ...
    $cmd                = "/usr/local/bin/setfiletypefilter.py";
    $cmd_init_tree      = "/usr/local/bin/getGroupUserTree.py";
    $cmd_file_type      = "/usr/local/bin/getfiletypejson.py";
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/file_type_filter.js"></script>
...
sub display_list_div() {
    ...
    <div id="panel_file_type"  style="width:96%;margin:20px auto"></div>
...
sub read_data(){
    ...
    # %ret_data->{'file_type_data'}={};
    # %ret_data->{'file_type_data'}{'id'}="all";
    # %ret_data->{'file_type_data'}{'text'}="所有";
    # %ret_data->{'file_type_data'}{'children'}=\@lines_file_type_names;
    my $file_json_str = `sudo $cmd_file_type`;
    #&debug_info($file_json_str."\n");
...
sub save_data() {
    ...
    }elsif($par{'ip_or_user'} eq 'sour_netip'){
        $par{'sour_netip_text'} = &datas_hander( $par{'sour_netip_text'},'save' );
    }
    if ($par{'dest_ipgroup'} eq 'dest_ip') {
        $par{'DestIPGroup'} = &datas_hander( $par{'DestIPGroup'},'save' );
    }elsif($par{'dest_ipgroup'} eq 'dest_group'){
        $par{'dest_ip_text'} = &datas_hander( $par{'dest_ip_text'},'save' );
    }
...
sub load_data(){
                ...
                if($search ne ""){
                    if (lc($config{'name'}) !~ m/$search/ &&
                     lc($config{'IpOrUser'}) !~ m/$search/ &&
                      lc($config{'target_ipgroups'}) !~ m/$search/ &&        
                       lc($config{'file_type'}) !~ m/$search/
                        # $config{'direction'} !~ m/$search/ &&
    ...
    my $ret = $json->latin1->encode(\%ret_data);
    print $ret; 
}
sub prepare_search($) {
    my $search = shift;
...
sub get_config_hash($) {
    ...
    $config{'DestIPGroup'}       = $temp[6];
    $config{'dest_ip_text'}      = $temp[7];
 
    $temp[8] =~ s/\|/\,\ /g;
    $config{'file_type'}      = $temp[8];
    $config{'direction'}      = $temp[9];
    $config{'protocol'}       = $temp[10];
    $config{'action_permission'} = $temp[13];
    $config{'is_record'}         = $temp[14];
    $config{'enable'}            = $temp[15];
...
sub get_config_record($) {
    ...
    my $SrcIPGroup = $hash_ref->{'SrcIPGroup'};

    $SrcIPGroup =~ s/，/&/g;
    push( @record_items, $SrcIPGroup );
    ...
    my $DestIPGroup = $hash_ref->{'DestIPGroup'};
    $DestIPGroup =~ s/，/&/g;
    ...
    push( @record_items, $hash_ref->{'direction'} );

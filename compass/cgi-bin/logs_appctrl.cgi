...
my $attacktypes_file = "/etc/firewall/ddosprotect/logtypeMapping";
my $appctrl_file = "/var/efw/objects/application/app_rule";
my %attacktypes;
my $log_name = "应用控制";
my %filterattacktypes;
my %appctrl_type;
my $service_hash;

my %chaindisplay;
$chaindisplay{'RETURN'}="允许";
...
sub init_data() {
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/logs_appctrl.js"></script>';
...
sub  do_action() {
    ...
    if ( $action eq 'load_data' && $panel_name eq 'list_panel' ) {
        ...
        ($errormessage,$notemessage) = &importLog(\%par,'appctrl');
        &show_page();
    }       
...
sub get_config_hash($) {
    ...
    my @filter_search_ex, $ex_search_flag = 0;
    foreach my $key (keys %filterchange) {
        my $temp_key = lc( $key ); #转为小写进行模糊查询  By WangLin 2015.04.09
        if ( $temp_key =~ m/$temp_filter/ ) {
        push( @filter_search_ex, "(".$filterchange{$key}.")" );
        $ex_search_flag++;
        }
    }
    foreach my $key ( keys %filterattacktypes ) {
        my $temp_key = lc( $key ); #转为小写进行模糊查询  By WangLin 2015.04.09
        if ($temp_key =~ $temp_filter) {

            push( @filter_search_ex, "(".$filterattacktypes{$key}.")" );
            $ex_search_flag++;
        }
    }
    if ( $ex_search_flag > 0 ) {
        $filter_search = join( "|", @filter_search_ex );
    }
    if ($filter eq "TCP" || $temp_filter eq "tcp" ) {
        $filter_search = $filter;
    }

    my @chaintemp=split(/\:/,$comment);
    my $attacktype_en = $chaintemp[0];
    $attacktype_en =~ s/\s//g;
    my $type = $attacktypes{ $attacktype_en };
    for(my $i=0;$i<2;$i++) {
        $chaintemp[$i]=~s/\s//g;
    ...
    $config{'srcprot'}     = $srcport;
    $config{'des'}         = $printdstaddr; 
    $config{'desport'}     = $dstport; 
    $config{'protocol'}    = $proto;  
    $config{'pro_type'}    = $type;
    $config{'rule_action'} = $chaintemp[1];

    $config{'app_type'} = '';
    foreach my $key ( keys %appctrl_type ) {
        $appid_key = int($appid);
        if ($key == $appid_key) {
            $config{'app_type'} = $appctrl_type{$key};
        }                                    
    }
    if ($config{'app_type'} eq '') {
        $proto = lc $proto;
        my $port_type = %$service_hash->{$proto};
        if ($port_type) {
           if (%$port_type->{$srcport}) {
                $config{'app_type'}    = %$port_type->{$srcport};
            } elsif(%$port_type->{$dstport}){
                $config{'app_type'}    = %$port_type->{$dstport};
            }else{
                $config{'app_type'}    = '未知';
            }
        }else{
...
sub load_data() {
    ...
    my $res = `sudo /usr/local/bin/getServiceMapJson.py`;
    my $file_json = $json->latin1->decode( $res );
...
sub get_config_record($) {
    ...
    my $searchTime = $hash_ref->{'searchTime'};
...
sub init_attacktypes() {
    my @lines = &read_conf_file( $attacktypes_file );
    foreach my $line ( @lines ) {
        chomp $line;
        $line =~ s/\n//g;
        $line =~ s/\r//g;
        my @temp = split( ",", $line );
        %attacktypes->{ $temp[0] } = $temp[1];
        %filterattacktypes->{ $temp[1] } = $temp[0];
    }
}

# 将应用类型组装
sub init_apptypes() {
    my @lines = &read_conf_file( $appctrl_file );
    foreach my $line (@lines) {
        chomp $line;
        $line =~ s/\n//g;
        $line =~ s/\r//g;
        my @temp = split(",", $line );
        %appctrl_type->{ $temp[0] } = $temp[1];    
    }
    # return %appctrl_type;

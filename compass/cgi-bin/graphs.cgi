...
;
	...
	foreach $graphname (@graphs) {
		if ($graphname =~ /RED_(.*)/) {
			...
			my $deal_date = deal_date($ftime);
			&openbox('100%', 'center', _("RED graph of uplink %s", $uplink)."-"._('The statistics were last updated at: %s', $deal_date));
			printf <<EOF
...
;
			...
			&openbox('100%', 'center', _("RED graph of uplink %s", $uplink));
			my $str = no_graph(_('No information available.'));
		print $str;
			}
		} else {
...
printf <<EOF
	<script>
		\$(document).ready(function() {
			\$(".containter-div-header span:nth-child(2)").remove();
		}); 
	</script>
EOF
;

...
sub init_data(){
    $custom_dir         = 'ipsconsole/child';
    $conf_dir           = '/var/efw/'.$custom_dir;
    $conf_file          = $conf_dir.'/console.config';
    ...
    $need_reload        = $conf_dir.'/sub_console_need_reload';
    $import_error_file  = $conf_dir.'/sub_console_import_error_file';
    $imported_tag       = $conf_dir.'/sub_console_imported_tag';
    $export_file        = '/tmp/sub_console.csv';
    $export_filename    = 'sub_console.csv';

    $restart                    = '';
    $distribute_strategy_cmd    = '/usr/local/bin/distributepolicy -c';

    $extraheader        = '<script language="JavaScript" src="/include/idps_sub_console_config_init.js"></script>
                            <script language="JavaScript" src="/include/template_add_list.js"></script>
                            <script language="JavaScript" src="/include/idps_sub_console_config.js"></script>
...
sub get_config_hash($) {
    ...
    #===================notice==============================
    $config{'sub_console_name'} = $temp[0];
    $config{'sub_console_ip'} = $temp[1];
    $config{'sub_console_connected'} = $temp[2];
    $config{'strategy'} = $temp[3];
...
sub get_config_line($){
    ...
    #===================notice==============================
    push(@data_arr, $data->{'sub_console_name'});
    push(@data_arr, $data->{'sub_console_ip'});
    push(@data_arr, $data->{'sub_console_connected'});
...
sub save_line_check($$){
    ...
    foreach my $line ( @$lines ) {
        ...
        if($temp{'sub_console_name'} eq $to_save_data{'sub_console_name'}) {
            $can_save = 0;
            $can_save_mesg = "已存在相同的控制台名称";
            last;
        }
    }
    #====end===============================================
...
sub update_line_check($$$){
    ...
    for(my $i = 0; $i < $length; $i++) {
        ...
        if($i != $line) {
            if($temp{'sub_console_name'} eq $to_update_data{'sub_console_name'}) {
                $can_update = 0;
...
sub distribute_line_check($$$){
    ...
    # if($temp{'sub_console_connected'} eq 'DISCONNECT') {
    #     $can_distribute = 0;
    #     $can_distribute_mesg = "$temp{'sub_console_name'}未连接,策略下发失败";
    # }
    #====end===============================================
...
sub save_data() {
    ...
    if($par{'line'} eq '') {
        ...
        $par{'sub_console_connected'} = "DISCONNECT";
        $par{'strategy'} = "";
        ...
        $par{'sub_console_connected'} = $temp[2];
        $par{'strategy'} = $temp[3];
        #======end by wl 2014.04.24=====================#
    }
    #==第二步保存==========================#
...
sub distribute_data() {
    ...
    &get_content_hash( \%consoles, $par{'consoles'} );
    ...
    for(my $i = 0; $i < $record_num; $i++){
        ...
        if ( $consoles{$i} eq "$i" ) {
            ...
            if($if_can){
                ...
                push(@console_names, $data{'sub_console_name'});
            }else{
                $fail_num++;
                push(@fail_mesg, $errormsg);
            }
    ...
    #=======根据引擎名字下发策略===========#
    my $consoles = join " ", @console_names;
    my $distribute_data_opt = $distribute_strategy_cmd." ".$consoles;
    ...
    if ( $fail_num == 0) {
        #==没有错误的信息===#
        $mesg = "$succeeded_num个子控制台成功下发策略";
    } else {
        $mesg = "[ $fail_num ]个下发失败,[ $succeeded_num ]个下发成功";
    }
    ...
    <div id="add-div" >
        <div id="add-div-header">
            <span class="add-header-title">
                ...
                <span id="add-title" class="label-text">添加子控制台</span>
            </span>
        </div>

...
sub init_data(){
    $access_log_file_path = "/var/log/frox/filetype.log";
    $base_dir = "/var/log/";
    $common_cmd = "/usr/local/bin/queryfilterlog.py";
    $log_search_tmp_file_path = "/tmp/ftp.log";
    #============扩展的CSS和JS文件-BEGIN========================================================#
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    <script language="JavaScript" src="/include/message_manager.js"></script>
                    <script language="JavaScript" src="/include/list_panel.js"></script>
                    <script language="JavaScript" src="/include/ftp_ftype_filter_log.js"></script>
...
sub show_page() {
    &openpage( "FTP文件类型过滤日志", 1, $extraheader );
...
sub display_main_body() {
    ...
    <div id="ftype_filter_log_panel" class="container"></div>
EOF
    ;
}
...
sub load_data(){
    ...
    if( $par{'INIT_TAG'} eq 'true' ){
        ...
        `echo 222 >/tmp/haha`;
        if( $par{'search'} ne ''){
            $query_cmd = $common_cmd." -f -m ".$formated_date." -g ".$par{'search'};
            `echo "$query_cmd" >>/tmp/haha`;  # /usr/local/bin/queryfilterlog.py -t -m 20150510 -g baidu
            system($query_cmd);
        }
        else{
            $query_cmd = $common_cmd." -f -m ".$formated_date;
...
sub get_config_hash($){
    ...
    $config{'SERVER_ADDR'}   = $temp[3];
    $config{'UP-DOWNLOAD'}  = $temp[4];
    $config{'FILE_NAME'}  = $temp[5];
...
sub delete_data(){
    ...
    my $formated_date = &get_format_date($par{'DATE'});
    my $cmd = $common_cmd." -f -e -m ".$formated_date;
    system($cmd);
    if($? == 0){
        $status = 0;
        $mesg = "日志删除成功";
...
sub export_data(){
    ...
    if( &is_today($formated_date) ){   `echo is_today-is-true >>/tmp/haha`;
        $export_file_path = $base_dir."frox.log";
        $file_name = "frox.log";
    }
    else{
        $export_file_path = $base_dir."frox.log-".$formated_date.".gz";
        $file_name = "frox.log-".$formated_date.".gz";
    }

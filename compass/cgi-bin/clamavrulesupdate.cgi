...
my $save_servername ;
my $old_version;
my $version_file_check;
my $config_file;
my $imported_file_update;
my $imported_file_check;
my $imported_file_dir;
...
sub init_data(){
    ...
    $imported_file_dir    = "/tmp/FWUpdateClamavRules.bin";
    $imported_file_check  = "/usr/local/bin/DecryptClamavRules";
    $imported_file_update = "/usr/local/bin/UpdateClamavRules";
    $save_update_online   = "/usr/local/bin/restartruleupdate";
    $update_system_online = "/usr/local/bin/AutogetUpdateClamav";
    $config_file          = "/var/efw/update/clamav/settings";
    $version_file_check   = "/var/license/clamav/unactivetag";
    $version_file         = "/var/efw/update/clamav/release";
    $old_version_file     = "/var/efw/update/clamav/release.old";
    $save_update_detail   = "/usr/local/bin/restartruleupdate -m clamav";
    $running_tag_file_path= "/var/efw/update/clamav/";
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script type="text/javascript" src="/include/clamavrulesupdate_controller.js"></script>
...
;
    if( !(-e $version_file_check) ){
        ...
        $clamav_version = <VERSIONFILE>;
        $updatetime = &get_file_mtime_by_formatday( "$version_file",'-' );
        close(VERSIONFILE);
        ...
        if(!$oldversion){
            $oldversion = $clamav_version;
        }
    }else{
        $clamav_version = "未 激 活";
        $updatetime = "未 激 活";
        $oldversion = "未 激 活";
    }
    printf <<EOF
    <tbody>
        <tr class="odd">
            <td width="20%" vlign="center" align="center">病 毒 库</td>
            <td width="20%" vlign="center" align="center">$clamav_version</td>
            <td width="20%" vlign="center" align="center">$updatetime</td>
...
sub display_offline_body() {
    ...
    printf <<EOF
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="font-size:12px;">
            <tbody><tr class="odd">
                <td width="20%" class='add-div-width' vlign='center' align='center' rowspan ='1'>病毒库升级：</td>
                <td width = "80%">
            ...
            <table  class="add-div-footer" width="100%" cellpadding="0" cellspacing="0" style="font-size:12px;">
                <tbody>
                <tr>
                <td width="78%""></td><td width="22%">请到<a onclick='jump("http://%s")' style='cursor:pointer' color='blue'>%s</a>下载最新升级包！</td></tr>
                </tr>
        </tbody></table>
        </form>
EOF

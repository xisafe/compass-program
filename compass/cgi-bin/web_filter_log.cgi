...
sub init_data(){
    $access_log_file_path = "/var/log/dansguardian/access.log";
    $base_dir = "/var/log/dansguardian/";
    $common_cmd = "/usr/local/bin/queryfilterlog.py";
    $log_search_tmp_file_path = "/tmp/dansguardian.log";
    #============扩展的CSS和JS文件-BEGIN========================================================#
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    <script language="JavaScript" src="/include/message_manager.js"></script>
                    <script language="JavaScript" src="/include/list_panel.js"></script>
                    <script language="JavaScript" src="/include/web_filter_log.js"></script>
...
sub show_page() {
    &openpage( "web内容过滤日志", 1, $extraheader );
...
sub display_main_body() {
    ...
    <!--<div id="web_filter_add_panel" class="container"></div>-->
    <div id="web_filter_log_panel" class="container"></div>
EOF
    ;
}
...
sub load_data(){
    ...
    if( $par{'INIT_TAG'} eq 'true' ){  # if( $par{'INIT_TAG'} eq 'true' or &is_today($par{'DATE'}) ){
        `echo 111 >/tmp/haha`;
        ...
        `echo 222 >/tmp/haha`;
        if( $par{'search'} ne ''){
            $query_cmd = $common_cmd." -t -m ".$formated_date." -g ".$par{'search'};
            `echo "$query_cmd" >>/tmp/haha`;  # /usr/local/bin/queryfilterlog.py -t -m 20150510 -g baidu
            system($query_cmd);
        }
        else{
            $query_cmd = $common_cmd." -t -m ".$formated_date;
...
sub get_config_hash($){
    ...
    $config{'LOG_TIME'}     = "$temp[0]"." "."$temp[1]";
    $config{'SRC_IP'}       = $temp[2];
    $config{'FILTER_URL'}   = $temp[3];
    $config{'FILTER_TYPE'}  = $temp[4];
    $config{'POLICY_NAME'}  = $temp[5];
    $config{'ACTION'}       = $temp[6];
    #============自定义字段组装-END===========================#
    return %config;
}
...
sub delete_data(){
    ...
    my $formated_date = &get_format_date($par{'DATE'});
    my $cmd = $common_cmd." -t -e -m ".$formated_date;
    system($cmd);
    if($? == 0){
        $status = 0;
        $mesg = "日志删除成功";
...
sub export_data(){
    ...
    my $formated_date = &get_format_date( $query{'DATE'} );
      ...
      # $formated_date的值是00，$file_name的值是access.log-00gz

    `echo "$formated_date" >/tmp/haha`;
    if( &is_today($formated_date) ){   `echo is_today-is-true >>/tmp/haha`;
        $export_file_path = $base_dir."access.log";
        $file_name = "access.log";
    }
    else{
        $export_file_path = $base_dir."access.log-".$formated_date.".gz";
        $file_name = "access.log-".$formated_date.".gz";
    }

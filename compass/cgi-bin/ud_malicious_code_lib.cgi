...
my $extraheader     = '<link rel="stylesheet" type="text/css" href="/include/ud_malicious_code_lib.css"/>
                        ...
                        <script type="text/javascript" src="/include/ud_malicious_code_lib.js"></script>';
my %par;
my %settings;
my %system_settings;
...
sub getVersion(){ #获取版本信息#
    my $temp_cmd = "/usr/local/bin/getUpdateRelease.py -t spite_rule";
...
sub save_conf() {
    ...
    $settings{'SNORT_RULES_URL'} = "http://".$update_server."/package/SpiteFeatureLib.bin";
    #===自动下发控制台===#
...
sub doaction() {
    ...
    if($par{'down-load'} ne ''){ #在线升级点击下载#
        ...
        if($self_url =~ /[a-zA-Z0-9:.\/]/){
            system("echo $self_url > /var/efw/unify_update/spite_rule/address");

            my $temp_cmd = "sudo /usr/local/bin/downloadUpdatePakage.py -u $self_url -n online_spite_rule.bin";
            my $res = `$temp_cmd`;
            if($res eq "yes"){
                $notemessage ="下载成功!";
            }else{
    ...
    if($par{'update_now'} ne ''){
        my $temp_cmd = "sudo /usr/local/bin/online_rules_update.py -t spite_rule -n online_spite_rule.bin";
        my $res = `$temp_cmd `;
        if($res eq "yes"){
                $notemessage ="升级成功!";
            }else{
    ...
    if($par{'import_rules'} ne '') {
        ...
        if(!$file || $file !~ /\.bin/) {
            ...
            if(!open(UPLOAD, ">$tmp_dir/SpiteFeatureLib.bin")) { $errormessage="打开文件失败"; return false; } 
            binmode UPLOAD;
            my $count = 0;
           ...
           my $temp_cmd = "sudo /usr/local/bin/unify_rules_update.py -t spite_rule -n SpiteFeatureLib.bin ";
           my $res = `$temp_cmd `;
            if($res eq "yes"){
                $notemessage ="升级成功!";
            }else{
...
sub import_rules() {
    ...
    system("$offline_update /tmp/SpiteFeatureLib.bin system >/dev/null &");
...
sub show_update_online(){
    ...
    $settings{'SNORT_RULES_URL'} =~ m{http://(.*)/package/SpiteFeatureLib.bin};
    ...
    my $self_url = `cat /var/efw/unify_update/spite_rule/address`;
    printf <<EOF

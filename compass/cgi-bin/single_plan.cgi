...
my $json = new JSON::XS;
#记录日志的变量
my $CUR_PAGE = "单次时间计划" ;  #当前页面名称，用于记录日志
...
sub init_data(){
    ...
    $conf_file          = $conf_dir.'/single';              #策略模板所存放的文件
    ...
    $cmd_ipgroups       = "/usr/local/bin/generateipgourp.py"; #调用编辑ip组脚本
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/single_plan.js"></script>';
...
sub get_config_hash($) {
    ...
    $config{'id'}           = $temp[0];
    $config{'name'}         = $temp[1]; 
    $config{'start_time'}   = $temp[2];
    $config{'end_time'}     = $temp[3];
...
sub get_config_record($) {
    ...
    my $finish = $end_date." ".$end_time_hour.":".$end_time_minute;
    push( @record_items, $finish );
    &debug_info(@record_items);

...
sub init_data(){
    ...
    $conf_file              = $conf_dir.'/custom/a94a8fe5ccb19ba61c4c0873d391e987982fbbd3.rules';
    $conf_filename          = "a94a8fe5ccb19ba61c4c0873d391e987982fbbd3.rules";
    ...
    $restart            = '';
    $load_data_cmd      = './idps_rules_helper.py -s "/var/efw/ips/rules/system_##_/var/efw/ips/rules/custom" ';
...
sub get_config_hash($) {
    ...
    my @temp = split( /\#\_\_\#/, $line_content );
    #===================notice==============================
    #  传入规则字段用','(逗号)隔开
    #  这里根据自定义字段@display_add_config_data配置返回哈希中的值
    #  也可以自定义,但要和get_config_line同步
    #===================notice==============================
    # if ( $line_content =~ /\(msg:"(.*)"; .* classtype:([\w\-\d_]+); sid:(\d+);/ ) {
...
sub load_data(){
    ...
    chomp($search);
    my @content_array = ();
    my $to_load_data_cmd = $load_data_cmd.'"'.$search.'"';#支持模糊查询
    my $to_load_data = `$to_load_data_cmd`;
    my @lines = split( /\_\#\#\_/, $to_load_data);
    my $record_num = @lines;
    my $total = 0;
...
sub toggle_enable($) {
    ...
    foreach my $conf_line ( @conf_lines ) {
        ...
        if( $par{ $conf_data{'sid'} } eq $conf_data{'sid'} ) {

...
sub init_data(){
    ...
    $conf_file          = $conf_dir.'/routeinfo';              #发件人信息所存放的文件
    $setting_enable_file= "/var/efw/dynamicroute/zebra/settings";
    $cmd_zebra          = "/usr/local/bin/restartzebra";
    $cmd_rip            = "/usr/local/bin/restartripd";
    $cmd_ospf           = "/usr/local/bin/restartospfd";
    $cmd_getroutinfo    = "/usr/local/bin/GetRouteInfo";
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/dynamic_route_init.js"></script>';
...
sub display_list_div() {
    printf<<EOF
    <div id="panel_dyroute_list" style="width: 96%;margin: 20px auto;"></div>
EOF
    ;
}
...
sub load_data(){
    system($cmd_getroutinfo);
    my %ret_data;
    my $content_array;
    my @lines = ();
    my ( $status, $error_mesg, $total_num ) = (0,"未读取",0);
...
sub get_config_hash($) {
    ...
    $config{'d_ip'}   = $temp[0];
    $config{'network_mask'}   = $temp[1];
    $config{'next_addr'}   = $temp[2];
    $config{'val_metric'}   = $temp[3];
    $config{'routeType'}   = $temp[4];
    $config{'interface'}   = $temp[5];
    #============自定义字段组装-END===========================#
    return %config;
}

...
my $create_ipsreport;           #ips后台报表生成程序
...
sub init_data(){
    ...
    $create_ipsreport = '/usr/local/bin/createipsreport';
    ...
    $restart = '';
    $extraheader = '<script language="JavaScript" src="/include/logs_report_task_list_init.js"></script>
                    <script language="JavaScript" src="/include/logs_report_task_list.js"></script>
...
sub save_data() {
    ...
    `$create_ipsreport $par{'report_name'} &`;
    &reset_values();
    #==========状态解释=========================#
...
sub create_display_add_lines(){
    ...
    for(my $i = 0; $i < $total_count; $i++){
        ...
        if($item->{'type'} eq 'text') {
            ...
            $input = '<input type="checkbox" name="'.$item->{'name'}.'" value="attack_type"/> 按入侵类型统计
            <input style="margin-left:10px" type="checkbox" name="'.$item->{'name'}.'" value="sourceip"/> 按源地址统计
            <input style="margin-left:10px" type="checkbox" name="'.$item->{'name'}.'" value="dip"/> 按目标统计
            <input style="margin-left:10px" type="checkbox" name="'.$item->{'name'}.'" value="danger_level"/> 事件级别
            <input style="margin-left:10px" type="checkbox" name="'.$item->{'name'}.'" value="affect_service"/> 影响服务
            <input style="margin-left:10px" type="checkbox" name="'.$item->{'name'}.'" value="action"/> 响应方式';
        }elsif($item->{'type'} eq 'cycle'){
            $input = '<input type="radio" name="'.$item->{'name'}.'" value="today" checked/> 今天
            ...
            $input = '<input type="checkbox" name="'.$item->{'name'}.'" id ="xsl" value="xls" checked="checked"/> EXCEL';
        }else{

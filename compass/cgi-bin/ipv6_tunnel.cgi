...
sub init_data(){
    $custom_dir         = '/ipv6/ipv6tunnel';
    $conf_dir           = "${swroot}".$custom_dir;
    $conf_file          = $conf_dir.'/config';
    $need_reload_tag    = $conf_dir.'/add_list_need_reload';
	$set_tunnel         = '/usr/local/bin/setipv6tunnel.py';#设置ipv6隧道py文件路径
	$set_test           = '/usr/local/bin/ipv6test.py';
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/ipv6_tunnel.js"></script>';
...
sub get_config_hash($) {
    ...
    $config{'name'}        = $temp[0];    
    $config{'type'}        = $temp[1];
	$config{'srcaddr'}     = $temp[2];
	$config{'destaddr'}    = $temp[3];
	$config{'description'} = $temp[4];
...
sub get_config_record($) {
	...
	push( @record_items, $hash_ref->{'srcaddr'} );
	push( @record_items, $hash_ref->{'destaddr'} );

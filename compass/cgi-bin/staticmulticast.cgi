...
sub init_data(){
    $custom_dir         = '/multicastrouting/staticmulticast';              #要保存数据的目录名字,要配置,不然路径初始化会出问题
    $conf_dir           = "/var/efw".$custom_dir;           #规则所存放的文件夹
    $conf_file          = $conf_dir.'/config';              #发件人信息所存放的文件
    $settings_file      = $conf_dir.'/settings';            #启用信息所存放的文件
    $need_reload_tag    = $conf_dir.'/add_list_need_reload_static';#启用信息所存放的文件
    $cmd_rip = "/usr/local/bin/StaticMulticast";
    $cmd_getinterface = "sudo /usr/local/bin/GetmulticastInterface.py";
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/staticmulticast_init.js"></script>';
...
sub display_list_div() {
    ...
    my $checked = "";
    if($settings{'ENABLED_STATIC'} eq 'on'){
        $checked = "checked = 'checked'";
    }
    printf<<EOF
    <div id="mesg_box_static" style="width: 96%;margin: 20px auto;"></div>
    <div id="enable_box_static" class="toolbar" style="width: 94.5%;margin: 20px auto;padding:10px;">
        <input type="checkbox" $checked value="on" id="enableStatic" />
        <span>启用静态组播路由</span>
        <button class="imaged-button" onclick="change_switch();">确定</button>
    </div>
    <div id="panel_static_add" style="width: 96%;margin: 20px auto;"></div>
    <div id="panel_static_list" style="width: 96%;margin: 20px auto;"></div>
EOF
    ;
}
...
sub get_config_hash($) {
    ...
    $config{'s_multicast'}   = $temp[0];
    $config{'addr_multicast'}   = $temp[1];
    $config{'input_multicast'}   = $temp[2];
    $config{'output_multicast'}   = $temp[3];
...
sub get_config_record($) {
    my $hash_ref = shift;
    my @record_items = ();
    push( @record_items, $hash_ref->{'s_multicast'} );
    push( @record_items, $hash_ref->{'addr_multicast'} );
    push( @record_items, $hash_ref->{'input_multicast'} );
    #my $selected_interface = $hash_ref->{'selected_interface'};
    my $selected_interface = $hash_ref->{'selected_interface_save'};
    $selected_interface =~ s/\|/\&/g;
    if($hash_ref->{'output_multicast'}){
        $selected_interface = $hash_ref->{'output_multicast'};
    }
    push( @record_items, $selected_interface );
...
sub make_file(){
    ...
    if(! -e $settings_file){
        ...
        `echo >>$settings_file ENABLED_STATIC=off`;
    }
}
sub get_all_interface(){
    my @all_hash;
    my $temp_cmd = `ip a`;
    my @temp = split(/\n/,$temp_cmd);
...
sub get_useable_interface(){
    ...
    # foreach(@ethes){
        ...
        # }
    # }
    my $str_interface = `$cmd_getinterface`;
    @useable_interfaces = split(",",$str_interface);
    foreach my $item (@useable_interfaces){
        chomp($item);
    }
    my %hash;
...
#启用禁用
sub switch(){
    $settings{'ENABLED_STATIC'} = $par{'ENABLED'};
    writehash($settings_file,\%settings);
    &create_need_reload_tag();
    &send_status( 0, 1, "操作成功" );
}
sub apply_data() {
    my $result;
    system($cmd_rip);

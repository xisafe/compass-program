...
my $logfile="/var/log/psad/top_attackers";
...
if (!$skip) {
    foreach my $line (<FILE>) {
		chomp($line);
		if ($line =~/^\d+/) {
			$line =~ /(\d+\.\d+\.\d+\.\d+)\s+(\d+)\s+\d+\s+\d+\s+\d+/;
			my $src_ip = $1;
			my $level = $2;
			if ($filter) {
				if ($src_ip =~ /$filter/ || $level =~/$filter/) {
					push (@log,$line);
				}
			}
			else{
...
<tr>
    <td width='15%' align='center' class='boldbase'><b>源IP</b>$sort_time</td>
    <td align='center' class='boldbase'><b>风险等级</b></td>
</tr>
END
;
if ($cgiparams{'status'} eq 'down') {

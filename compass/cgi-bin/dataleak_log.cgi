...
sub init_data(){
    ...
    $extraheader = '<link rel="stylesheet" type="text/css" href="/include/add_list_base.css" />
                    ...
                    <script language="JavaScript" src="/include/dataleak_log.js"></script>';
...
sub do_action() {
    ...
    if ( $action eq 'load_data' && $panel_name eq 'list_panel' ) {
		...
		&clearLog_in_dataBase(\%par, 'dataleak_log');
        &show_page();
    }
}
...
sub load_data_extend() {
	...
	unless($data eq "") {
		...
		if($search ne "") {
			...
			my $cmd_temp = "sudo /usr/local/bin/findlog.py -n 'dataleak_log' -f $search -t $time";
            my $result = `$cmd_temp`;
           if($result){

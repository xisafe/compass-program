...
RuleAddPanel.prototype.control_display_for_popup_box = function(ele,status) {
    ...
    var panel = document.querySelector('#'+rule_add_panel.panel_control.panel_id);
    var panel_site = rule_add_panel.get_ele_left_top(panel);
    if (status == 'show') {
        $(popup_cont_id).css({
            left: ele_site.left-panel_site.left+28,
            top: ele_site.top-panel_site.top-8-document.querySelector('#'+rule_add_panel.panel_control.panel_id+' .container-main-body').scrollTop,
...
RuleAddPanel.prototype.create_popup_item_detail = function(item) {
    var rule_add_panel = this;
...
RuleAddPanel.prototype.create_widget_tip = function(item) {
    ...
    widget_tip += "<div class='popup-tip' onclick='$(this).hide()' "

    var popup_obj = rule_add_panel.get_input_check_obj('popup');
    var popup_item = '';

        var attr_obj = {
            id : 'popup_item_id_for_' + item.name,
            class : 'popup-tip'
        }
        popup_item += rule_add_panel.add_attr_to_item(attr_obj);
        popup_item += '><ul class="cont" >';
        var popup_item_detail ='';
        if (item.check || item.popup_cont != false) {
            popup_item_detail = rule_add_panel.create_popup_item_detail(item);
        }
        popup_item += popup_item_detail ;
        popup_item += '</ul><b class="out" ></b><b class="in" ></b></div>';
    

    widget_tip += popup_item

...
function generate_list_main_body() {
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        for ( var j = 0; j < display_cols.length; j++) {
            if(display_cols[j] == "port"){
                var temp_arr = detail_data[i][display_cols[j]].split("&");
                table_main_body += '<td>';
                for(var k=0;k<temp_arr.length;k++){
                    table_main_body += (temp_arr[k]+'<br/>');
                }
...
function update_main_body_info() {
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        for(var j = 1; j < display_cols.length; j++) {
            
            listb_tr.eq(tr_num).find("td").eq( j+1 ).html(detail_data[i][display_cols[j]]);
        }
        var actions = '<input type="image" class="action-image" src="../images/edit.png" title="编辑" value="' + data_item_id + '" onclick="edit_item(this);"/>';
...
function edit_item(element) {
    ...
    document.getElementById("bgDiv2").style.display = "";
    ...
    var name_edt = document.getElementById("name_edt");
    name_edt.value=data_item.name_cn;
    var port_edt = document.getElementById("port_edt");
    var array_port = data_item.port.split("&");
    var port_show = array_port.join("\r\n");
    port_edt.value=port_show; 

    /*对表单中的值重新做检查*/
    //check._submit_check(object,check);
}

function save_data_edt(){
    var name_save = document.getElementById("name_edt").value;
    var port = document.getElementById("port_edt").value;
    var ports = port.split("\n");
    for(var i=0;i<ports.length;i++){
        var reg = /^([1-9][0-9]*)$/;
        var port_range = ports[i].split(":");
        if(port_range.length == 2){
            if(/^([1-9][0-9]*)[:|-]([1-9][0-9]*)$/.test(ports[i])){
                if(parseInt(RegExp.$1)>65535||parseInt(RegExp.$1<0)||parseInt(RegExp.$2)>65535||parseInt(RegExp.$2<0)){
                    alert(ports[i]+"中的端口应在0-65535之间");
                    return;
                }
                if(parseInt(RegExp.$1) >= parseInt(RegExp.$2)){
                    alert(ports[i]+"中的端口前一个应该小于后一个");
                    return;
                }
            }else{
                alert(ports[i]+"端口范围格式错误");
            }
        }else{
            if(!(reg.test(ports[i])&&RegExp.$1<=65535&&RegExp.$1>=0)){
                alert("您修改的"+ports[i]+"端口格式不正确！");
                return;
            }
        }
    }
    var port_save = ports.join("&");
    
    var sending_data = {
        ACTION: "save_data_edt",
        name_save: name_save,
        port_save:port_save
    };
    function ondatareceived(data) {

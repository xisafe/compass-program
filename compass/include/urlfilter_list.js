...
function generate_list_main_body() {
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        table_main_body += '<input type="checkbox" class="rule-listbcc" value="' + detail_data[i].line + '" onclick="toggle_item_check(this);"/>';
        table_main_body += '</td>';
        for ( var j = 1; j < display_cols.length+1; j++) {
            table_main_body += '<td>';
            if(display_cols[j] == 'blacklist_user' || display_cols[j] == 'blacklist_url'){
                var temp_arr = detail_data[i][display_cols[j]].split("|");
                var temp_str = temp_arr.join("\n");
                table_main_body += temp_str;
            }else{
                table_main_body += detail_data[i][display_cols[j]];
            }
            table_main_body += '</td>';
...
function update_main_body_info() {
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        for(var j = 0; j < display_cols.length; j++) {
            if(display_cols[j] == 'blacklist_user' || display_cols[j] == 'blacklist_url'){
                var temp_arr = detail_data[i][display_cols[j]].split("|");
                var temp_str = temp_arr.join("<br/>&nbsp;&nbsp;");
                listb_tr.eq(tr_num).find("td").eq( j + 1 ).html(temp_str);
            }else{
...
function edit_item(element) {
    ...
    for(var i = 0; i < input_textareas.length; i++) {
        var arr_inputarea = data_item[input_textareas[i].name].split("|");
        var val_inputarea = arr_inputarea.join("\n");
        input_textareas[i].value = val_inputarea;

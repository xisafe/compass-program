 ...
 * 第一步，定义全局变量
 */
var agreement_type = "tcp";
var error_tcp = 0;
var error_udp = 0;
var error_icmp = 0;
var ass_url = "/cgi-bin/custom_service.cgi";
...
var add_panel_config = {
    ...
    rule_title: "自定义服务", 	/* ==*可选*==，默认是"规则" */
    render: add_panel_render,
    ...
    event_handler: {                /* ===可选===，如果要在一些节点做事情，可以在此定义如下一些函数 */
        ...
        after_load_data: function( add_obj, data_item ) {
            ...
            var icmp = data_item.icmp_textarea;
            var icmp_array = icmp.split("\n");
            var temp= "";
            for (var i = 0; i < icmp_array.length; i++) {
                icmp_array[i] = icmp_array[i].replace(/type\:/g,"");
                icmp_array[i] = icmp_array[i].replace(/,/g,"\/");
                icmp_array[i] = icmp_array[i].replace(/code\:/g,"");
                var temp = temp +  icmp_array[i] + "\n";
                // return icmp_array[i] + "\n";
            };
            $("#icmp_textarea").val(temp);
        },
					...
					sub_items: [
						{
							...
							id: "tcp_button",
							name: "tcp_button",
							class: "agreement_button",
							value: "TCP",
							functions: {
								onclick: "agreement_button_click('tcp');"
							}
							...
							value: "UDP",
							functions: {
								onclick: "agreement_button_click('udp');"
							}
						},{
							enable: true,
							type: "button",
							...
							value: "ICMP",
							functions: {
								onclick: "agreement_button_click('icmp');"
							}
						},{
							enable: true,
							type: "button",
							...
							value: "其他",
							functions: {
								onclick: "agreement_button_click('other');"
							}
						}
					...
					sub_items: [
						{
							...
							id: "tcp_textarea",
							name: "tcp_textarea",
							class: "agreement_textarea",
                            tip: "(端口号:1-65535,一行一个)",
							value: "",
							check: {
								type: "textarea",
								required: 0,
								check: 'note|',
								ass_check: function( check ) {
									var str = $("#tcp_textarea").val();
									var arr = str.split("\n");
									// console.log(arr);
									for(var i=0; i<arr.length; i++){
										var error = checkTcpUdp(arr[i],"不是正确的TCP格式!");
										if(error != "") {
                                            error_tcp = 1;
											return error;
										}
							...
							check: {
								...
								check: 'note|',
								ass_check: function( check ) {
									var str = $("#udp_textarea").val();
									var arr = str.split("\n");
									for(var i=0; i<arr.length; i++){
										var error = checkTcpUdp(arr[i],"不是正确的UDP格式!");
										if(error != "") {
                                            error_udp = 1
											return error;
										}
							...
							name: "icmp_textarea",
							class: "agreement_textarea",
                            tip: "格式：type[/code]<br/>回显应答（Ping应答）: 0<br/>回显请求（Ping请求）: 8<br/>主机不可达: 3<br/>端口不可达: 3/3",
							style: "display:none;",
							value: "",
							check: {
								type: "textarea",
								...
								check: 'note|',
								ass_check: function( check ) {
									var str = $("#icmp_textarea").val();
									var arr = str.split("\n");
									for(var i=0; i<arr.length; i++){
										var error = checkIcmp(arr[i]);
										if(error != "") {
                                            error_icmp = 1;
											return error;
										}
...
function agreement_button() {
	var $button = $(".agreement_button");
	$button.css("margin","0");
	$button.css("padding","0");
	$button.parent().css("margin","5px 1px");
	
}

function agreement_textarea() {
	var $textarea = $(".agreement_textarea");
	var $textarea_parent = $textarea.parent();
	$textarea.css("","");
	$textarea.css("","");
	$textarea.css("","");
	$textarea.css("","");
	$textarea_parent.css("margin","0");
	$textarea_parent.css("","");
	$textarea_parent.css("","");
	$textarea_parent.css("","");
...
function agreement_button_click(str) {
	var str1 = str + "_button";
	var str2 = str + "_textarea";
	
    $("#other_textarea").css('display','none');
	$(".agreement_button").css("height","20px");
	$(".agreement_textarea").css("display","none");
	$(".agreement_textarea").next().css("display","none");
	$(".agreement_textarea").next().next().css("display","none");
	$("#" + str1 + "").css("height","29px");
	$("#" + str2 + "").css("display","inline-block");
    $("#" + str2 + "").next().css("display","inline-block");
	$("#" + str2 + "").next().next().css("display","inline-block");

    var height = document.getElementById('modal_border_box_add_panel').offsetHeight;
    document.getElementById('TransparentBorder_add_panel').style.height = height + 'px';
	
}

//添加编辑页面渲染
function addpanel_render() {
	agreement_button();
	agreement_textarea();
	$("#tcp_button").click();
...
function beforeSavedata() {
    ...
    arr[0] = $("#tcp_textarea").val();
    arr[1] = $("#udp_textarea").val();
    arr[2] = $("#icmp_textarea").val();
    arr[3] = $("#other_textarea").val();
	var all = arr.join("");
	var tcp = arr[0].split("\n");
	var udp = arr[1].split("\n");
	var icmp = arr[2].split("\n");
	var other = arr[3].split("\n");
	all = all.replace(/\s/g,"");
	if(all == "") {
		list_panel.show_error_mesg("协议不能为空");
		return false;
	}
    if (error_tcp == 1) {
        list_panel.show_error_mesg("请正确填写TCP协议");
        return false;
    }
    if (error_udp == 1) {
        list_panel.show_error_mesg("请填写正确UDP协议");
        return false;
    }
    // if (error_icmp == 1) {
    //     list_panel.show_error_mesg("请填写正确的ICMP协议");
    //     return false;
    // }
...
//检测ICMP
function checkIcmp(str) {
    var flag;
    if(str === "any") {
        return "";
    }
    var error = "不是正确的ICMP格式!";
    // var re = /^type:((([1-9][0-9]*|0)-([1-9][0-9]*))|([1-9][0-9]*|0))($|,code:((([1-9][0-9]*|0)-([1-9][0-9]*))|([1-9][0-9]*|0)))/;
	var re = /^([0-9]*|[0-9]*\/[0-9]*)$/;
    if(!re.test(str)) {
        flag = error;
		// return error;
 ...
 //        arr[0] = arr[0].split(":");
	// 	if(/-/.test(arr[0][1])){
	// 		var num = arr[0][1].split("-");
	// 		if(num[0]<0||num[0]>40||num[1]<0||num[1]>40||num[0]>=num[1]) {
	// 			return error;
	// 		}
	// 	}else {
	// 		if(arr[0][1]>40||arr[0][1]<0) {
	// 			return error;
	// 		}
	// 	}
	// 	if(arr.length == 2) {
	// 		arr[1] = arr[1].split(":");
	// 		if(/-/.test(arr[1][1])){
	// 			var num = arr[1][1].split("-");
	// 			if(num[0]<0||num[0]>16||num[1]<0||num[1]>16||num[0]>=num[1]) {
	// 				return error;
	// 			}
	// 		}else {
	// 			if(arr[1][1]>16||arr[1][1]<0) {
	// 				return error;
	// 			}
	// 		}
	// 	}
...
//检测TCP和UDP
function checkTcpUdp(str,error) {
	var reInt = /^([1-9][0-9]*|0)$/;
	var flag;
	if(!reInt.test(str)) {
		flag = error;
...
function other_protocol (){
    var sending_data = {
        ACTION: "load_other_protocol",
    }

    function ondatareceived(data) {
        $("#other_textarea").empty();
        $("#other_textarea").append("<option value=''>无</option>");
        for (var i=0; i<data.other_protocol.length; i++) {
            $("#other_textarea").append("<option value='"+data.other_protocol[i]+"'>"+data.other_protocol[i]+"</option>");
        };
    }

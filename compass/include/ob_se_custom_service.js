...
var ass_url = "/cgi-bin/ob_se_custom_service.cgi";
...
var add_panel_config = {
    ...
    event_handler: {                /* ===可选===，如果要在一些节点做事情，可以在此定义如下一些函数 */
        ...
        after_load_data: function( add_obj, data_item ) {
            var string = data_item.string.replace(/\&/g,'\n');
            $('#string').val(string);
            /*
            ...
            sub_items: [
                {
                    ...
                    popup:["用于识别特征ASCⅡ码，长度在4-48个字符","如www.sogou.com(特征串不支持中文)"],
                    value: "",
                    check: {
                        type: "text",
                        required: 1,
                        check: 'other|',
                        other_reg: '/^[^\\u3400-\\u4DB5\\u4E00-\\u9FA5\\u9FA6-\\u9FBB\\uF900-\\uFA2D\\uFA30-\\uFA6A\\uFA70-\\uFAD9\\uFF00-\\uFFEF\\u2E80-\\u2EFF\\u3000-\\u303F\\u31C0-\\u31EF,]+$/',
                        ass_check: function( check ) {
                            var str_len = $('#string').val().length,msg=''
                            if (str_len<4||str_len>48 ) {
                                msg='特征串长度应在4-48个字符'
                            }
                            return msg
                        }
    ...
    panel_header: [                 /* ***必填***，控制数据的加载以及表头显示 */
        {
            ...
            td_class: "align-center",           /* ==可选==，作用于整列，控制单元格中内容显示样式，比如要求内容居中显示，首行缩进两字符等

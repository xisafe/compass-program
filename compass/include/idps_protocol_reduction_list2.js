...
function load_search_data(){
    ...
    function ondatareceived(data) {
        ...
        if (is_success(data_content.status) || data_content.detail_data.length > 0){
            hide_loading_mesg();
            ...
            paging_holder.data_content.detail_data = [];
            paging_holder.data_content.display_cols = data_content.display_cols;
            paging_holder.data_content.total = 0;
...
function change_action(){
    ...
    var text_btn_caught = document.getElementById("btn_caught").value;
    //var text_btn_caught = document.getElementById("text_btn_caught").innerHTML;
    var sending_data_start = {
        ACTION: "caught_start",
        protocol: protocol
    };
    
    var sending_data_stop = {
        ACTION: "caught_stop",
        protocol: protocol
    };

    function ondatareceived(data) {
        var protocol_selected = data;
        //document.getElementById("protocol").value = protocol_selected;
        var sel = document.getElementById("protocol");
        for(var i=0;i<sel.options.length;i++){
            if(sel.options[i].value == protocol_selected){
            sel.options[i].selected = true;
            }
        }
    }
    if(text_btn_caught == "开始抓包"){
        //document.getElementById("text_btn_caught").innerHTML = "停止抓包";
        do_request(sending_data_start, ondatareceived);
        document.getElementById("btn_caught").value = "停止抓包";
    }else{
        //document.getElementById("text_btn_caught").innerHTML = "开始抓包";
        do_request(sending_data_stop, ondatareceived);
        document.getElementById("btn_caught").value = "开始抓包";
    }
    /* var sel = document.getElementById("protocol");
    for(var i=0;i<sel.options.length;i++){
        if(sel.options[i].value == protocol){
            sel.options[i].selected = true;
        }
    }  */
}
...
function track_source(e){
    ...
    function ondatareceived(data) {
        ...
        disable_btn();
        window.setTimeout("load_search_data()",1000);
        //load_search_data();
    }
...
function last_page(){
    ...
    if(current_page>1){
     ...
     if(current_page == 1){
            $("#last_page_icon").attr("disabled","disabled");
        }else{
            if(current_page < parseInt($("#total-page").html())){
                $("#last_page_icon").removeAttr("disabled");
                $("#next_page_icon").removeAttr("disabled");
            }
        }
    ...
    function ondatareceived(data) {
        ...
        var page_now = parseInt(content.mesg);
        disable_btn();
        load_search_data();
        $("#current-page").val(page_now);
        // if(page_now == 1){
        //     $("#last_page_icon").attr("disabled","disabled");
        // }else{
        //     if(page_now < parseInt($("#total-page").html())){
        //         $("#last_page_icon").removeAttr("disabled");
        //         $("#next_page_icon").removeAttr("disabled");
        //     }
        // }
...
function next_page(){
    ...
    function ondatareceived(data) {
        ...
        $("#current-page").val(page_now);
        if(page_now >= parseInt($("#total-page").html())){
            $("#next_page_icon").attr("disabled","disabled");
        }else{
            if(page_now > 1){
                $("#last_page_icon").removeAttr("disabled");
            }
...
function goto_page(){
    ...
    //var current_page = paging_holder.current_page;
    var page_goto = parseInt(document.getElementById("page_togo").value);
    paging_holder.current_page = page_goto;
    ...
    function ondatareceived(data) {
        //document.getElementById("btn_go").disabled = true;
        var content = JSON.parse(data);
        var page_now = content.mesg;
        disable_btn();
        load_search_data();
        $("#current-page").val(page_now);
        if(page_now == $("#total-page").html() && page_now == 1){
            $("#next_page_icon").attr("disabled","disabled");
            $("#last_page_icon").attr("disabled","disabled");
        }
    }
    ...
    for (var i = from_num; i < to_num; i++) {
        ...
        for ( var j = 0; j < display_cols.length; j++) {
            ...
            if(display_cols[j] == 'source_ip'){
                ...
                table_main_body += '</td>';
            }else if(display_cols[j] == 'content'){
                table_main_body += '<td style="word-break:break-all;">';
                table_main_body += (detail_data[i][display_cols[j]]);
                table_main_body += '</td>';
            }else{
                table_main_body += '<td>';

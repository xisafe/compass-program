...
$(document).ready(function(){
    ...
    net_flow_chart = echarts.init(document.getElementById('net_flow_chart')); //生成饼图
    ...
    if ( data_obj == null || data_obj.net_charts_obj == null){
     message_manager.show_error_mesg("未设置网站类型统计条件");
    }else{
        data_obj = data_obj.net_charts_obj;
    $.each(data_obj,function(i,v){
        ...
        if(data_obj[i]['value'] == 1){   /*判断统计数据是否是网站类型*/
...
var message_box_config = {
    url: "/cgi-bin/net_classify_flow_charts.cgi",
 ...
 var panel_header_for_type = [
       {
            "enable": true,
            "type": "text",
            "title": "网站类型",        //一般text类型需要title,不然列表没有标题
            "name": "name",
            "width": "10%" ,
...
var member_panel_config = {
    url: "/cgi-bin/net_classify_flow_charts.cgi", /* ***必填***，控制数据在哪里加载 */
...
var list_panel_config = {
    url: "/cgi-bin/net_classify_flow_charts.cgi", /* ***必填***，控制数据在哪里加载 */
    check_in_id: "panel_flow_list_for_net",         /* ***必填***，确定面板挂载在哪里 */
    page_size: 3,                           /* ===可选===，定义页大小，默认是15 */
    panel_name: "list_panel_for_url",       /* ==*可选*==，默认名字my_list_panel，当一二个页面存在多个相同的面板，此字段必填，以区别不同面板 */
    // panel_title: "IP/用户",
...
function load_chart_data(obj,updateListPanel){
    ...
    if(obj.length == 0){
        var nodata = [];
	message_manager.show_error_mesg("未设置网站类型统计条件");
        // net_flow_option.noDataLoadingOption.text = "未设置统计条件";
        renderNetTypeChart(nodata);
    }
    else{
        ...
        if(send_obj.hasOwnProperty("ruser")){
            ...
            var classify_value = send_obj.utext;

            $("#url_type").html("("+classify_value+")");
	    
	    
        net_flow_chart.showLoading({
        text:"加载中",
        effect:"whirling",
        textStyle : {
            fontSize : 20
        ...
        function ondatareceived(data){
            ...
            if( istrend == 0){
                // net_flow_option.noDataLoadingOption.text = "暂无数据";
                if (data_obj[1].value == 'live') {
                    renderNetTypeChart(_data,'live');

                }else{
               	    renderNetTypeChart(_data);
                }

            }
            else{

                renderNetTypeTrendChart(data.detail_data,top);
            }
            if(updateListPanel){
                if(_data.length > 0){
                    $("#no_data_box").hide();
...
function renderNetTypeChart(detail_data,status){
    ...
    net_flow_option.legend.data = legend_data;
    net_flow_option.series =[ {
        name:'总流量',
        type:'pie',
        radius : '50%',
        startAngle:  0 ,
    ...
    net_flow_chart.hideLoading();
    net_flow_chart.setOption(net_flow_option,true);
...
function renderNetTypeTrendChart(detail_data,top){
   ...
   var array = objectToArray(o);
    ...
    net_flow_trend_option.xAxis[0].data = xaxis_data;
    net_flow_trend_option.legend.data = legend_data;
    net_flow_trend_option.series =series_data;
    net_flow_chart.clear();
    net_flow_chart.hideLoading();
    net_flow_chart.setOption(net_flow_trend_option,true);
    
}


//显示饼图
function showNetTypeChart(){
...
// 显示趋势图
function showNetTypeTrendChart(){

...
var ass_url = "/cgi-bin/ipv6_tunnel.cgi";
...
var add_panel_config = {
    ...
    items_list: [                   /* ***必填***，确定面板中有哪些规则，不填什么都没有 */
        {
            title: "隧道名称",          /* **必填**，每一行前面的提示文字，不填没有提示，影响用户体验 */
            id: "name_tr",          /* ==可选==，整行的id */
            name: "",               /* ==可选==，整行的name */
            class: "",              /* ==可选==，整行的class */
...
var list_panel_render = {
    ...
    'action': {
        ...
        var button_icon,button_text,onclick;
        var action_buttons = [];
        if( enable == 1 ){
            button_icon = "on.png";
            button_text = "禁用";
            onclick = "toggleEnable(0," + data_item.id + ")";

        }
        else{
            button_icon = "off.png";
            button_text = "启用";
            onclick = "toggleEnable(1," + data_item.id + ")";
    ...
    top_widgets: [                          /* ===可选===，在面板左上角的操作控件，目前按钮位置不能调整 */
        {
            ...
            button_text: "添加",        /* **必填**，image_button的文字，这个必须设置,建议在五个字以内，image_button独有字段 */
...
function add_rule( element ) {
    add_panel.show();
    console.log(add_panel.get_input_check_obj());
}
...
//修改启用禁用
function toggleEnable(enable,id) {
    console.log(id);
    var sending_data = {
        ACTION: "toggleEnable",
        enable: enable,

            ...
            sub_items: [
                {
                    ...
                    check: {
                        ...
                        ass_check: function( check ) {
                            // var str = $("#nat64").val();
                            // var reg = /^(32|40|48|56|64|96)$/;
                            // if(!reg.test(str)) {
                                // return "请输入正确的nat64前缀格式!"
                            // }
...
function listPanelRender() {
    var select$ = $("#ipv4");
    select$.html('<option selected="selected" disabled="disabled">请选择ipv4上行接口</option>');
    load_ipv4();
}
...
function load_ipv4() {
    ...
    var ondatareceived = function(data) {
        var ipv4 = data.detail.ip_address;
        $("#ipv4").append("<option value=" + ipv4 + ">" + ipv4 + "</option>")
    }

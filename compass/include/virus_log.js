...
var ass_url = "/cgi-bin/virus_log.cgi";
...
function formatBytes(bytes){
    ...
    return  ((bytes/obj[type][0]).toFixed(fix).toString() + obj[type][1]).replace(/\.0*$/,'')
}
...
var list_panel_config = {
    ...
    render:{
        ...
        //          return '<span  onclick="show_suggestion(\'' + data_item.virus_name+'\',\''+data_item.id+ '\')">' + default_rendered_text + '<img src="/images/suggestion.png" class="suggestion-img" alt="点击查看MD5" title="点击查看MD5"/></span>';
        //     }
        // },
        'file_size':{
...
function show_suggestion( v,f ) {
    ...
    function ondatareceived(data) {
		var string = data.mesg;
		var stringcontent = "MD5:" + string; 
        message_manager.show_details_message( stringcontent , true);
     
      // var array[] = data.detail_data ; 
      // var i = data.id;
      // message_manager.show_details_message( array[i].virus_md5 , true);
...
function import_panel_extendrender(argument) {
    ...
    .wrap('<form enctype="multipart/form-data" method="post" action="/cgi-bin/virus_log.cgi" style="display:inline-block"></form>');

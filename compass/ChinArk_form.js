...
ChinArk_forms.prototype = {
	...
	_error_tip: function(msg, width, height,name) {
		...
		var html = '<div class="popup-tip" onclick="$(this).hide()" id="popup_item_id_for_'+name+'"'
		html += 'style="border-radius: 7px;background-color: #6BC6E2;cursor: pointer;position: absolute;z-index: 9999;">'
	...
	_tip: function(option, name, msg, form_name) {
		...
		if (msg) {
			if ($(popup_item_id).length != 0) {
				...
				container.style.top = ele_site.top +'px';
				container.style.left = ele_site.left+$(ele).width()+ 8 +'px';
